...
module.exports = (req, res) => {
  ...
  console.log("Starting ----------------Accept Video Url----------------------------");

  const listingSignature = req.body.sig;
  const listingId = req.body.listingId;
  const sellerId = req.body.sellerId;
  const influencerId = req.body.influencerId;
  const productVideoUrl = req.body.productVideoUrl;
  

  console.log("productVideoUrl --------------------------------------------    "+productVideoUrl);
    ...
    console.log("Calling ------------------------------------------------------------");
     ...
     try{
         if(parseInt(obj[key]) !== undefined && obj[key].sig === listingSignature){
           ...
           obj[key].status5SendVideoUrl = true;
           obj[key].status0NextStatus = "ConfirmVideoUrlReciept";
           obj[key].productVideoUrl = productVideoUrl;
